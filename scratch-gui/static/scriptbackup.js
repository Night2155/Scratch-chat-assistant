...
const guiUrl = 'http://ct.easylearn.org:8060/';
...
function getDbFile() {
    ...
    const logsFileRef = firebase.storage().ref(`${localStorage.identity}/${classno}/${userno}/Projects/${ProjName}/${username}_${ProjName}.csv`);
            ...
            fetch(foundURL)
                .then(function (response) {
                    response.text().then(function (text) {
                        storedText = text;
                        create('\uFEFF' + storedText + '\n\n' + getDate() + ',' + getTime() + ',專案,接續專案：' + ProjName + logs.join(' '),
                            username + '_' + ProjName + '_' + new Date().toLocaleString() + '.csv',
    ...
    function onReject(error) {
        ...
        create('\uFEFFDate,Time,Action,Details\n' + getDate() + ',' + getTime() + ',專案,第一次新增專案：' + ProjName + logs.join(' '),
            username + '_' + ProjName + '_' + new Date().toLocaleString() + '.csv',
            'text/csv;charset=utf-8');
    }
}
...
function create(text, name, type) { // create logs file
    ...
    var storageRef = storage.ref(`${localStorage.identity}/${classno}/${userno}/Projects/${projName}/${username}_${ProjName}.csv`);
    storageRef.put(file);
}
...
function setToken() {
    ...
    window.location.href = `${guiUrl}?token=${localStorage.token}&classno=${localStorage.classno}&no=${localStorage.no}&name=${localStorage.username}&p=${ProjName}&i=${localStorage.identity}`;
}