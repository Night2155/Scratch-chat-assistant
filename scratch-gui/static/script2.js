...
function Timeout2() {
    ...
    logs.push(`\n${getDate()},${getTime()},閒置儲存,閒置120秒儲存`);
}
function ReCalculate2() {
    clearTimeout(oTimerId2);
    oTimerId2 = setTimeout('Timeout2()', 2 * 60 * 1000);
}
document.onmousedown = ReCalculate2;
ReCalculate2();

const siteUrl = 'https://mmn.easylearn.org/';
const guiUrl = 'http://localhost:8060/';
...
$(document).ready(function () {
    ...
    $(document).on("keydown", enableSpace);
    checkExample();
    ...
    if (identity == "t") {
        document.getElementsByClassName("menu-bar_menu-bar-item_oLDa- menu-bar_hoverable_c6WFB")[1].style.display = 'none';
        document.getElementsByClassName("menu-bar_menu-bar-item_oLDa- menu-bar_hoverable_c6WFB")[2].style.display = 'none';
        document.getElementsByClassName("menu-bar_menu-bar-item_oLDa- menu-bar_hoverable_c6WFB")[3].style.display = 'none';
    }
    document.title = urlParams.get('p');
});
    ...
    if ((e.which || e.keyCode) == 32) {
        ...
        Object.keys(localStorage)
            .forEach(function (key) {
                if (/^sprite:/.test(key)) {
                    console.log('\n' + key + '\n ' + localStorage[key]);
                    logs.push(`\n${getDate()},${getTime()},執行,點擊透過空白鍵執行 ${key}工作區：${localStorage[key]}`);
                }
            });
    }
};
...
function checkExample() {
    if ((example != null) && ((ProjName == null) || (ProjName == '') || (ProjName == 'null'))) {
        do {
    ...
    } else if ((example != null) && ((ProjName != null) || (ProjName != '') || (ProjName != 'null'))) {
...
function checkLoadProjName(ProjName) {
    ...
    var loadProjName = document.getElementsByClassName('menu-bar_title-field-growable_3qr4G')[0].value;
    if (loadProjName != nowProjName) {
        console.log(`${getDate()},${getTime()},讀取專案,${loadProjName}`);
        logs.push(`\n${getDate()},${getTime()},讀取專案,${loadProjName}`);
    }
}
...
function createUI() { // must remove UI first, otherwise the sequence will be wrong
    ...
    */
    ...
    projLable.textContent = "專案名稱： " + urlParams.get('p');
...
function clickUI(targetElement) {
    ...
    if (targetElement.id == "downloadSB3") {
        ...
        logs.push(`\n${getDate()},${getTime()},儲存專案,${loadProjName}`);
    ...
    if (targetElement.className == "green-flag_green-flag_1kiAo") {
        ...
        Object.keys(localStorage)
            .forEach(function (key) {
                if (/^sprite:/.test(key)) {
                    console.log('\n' + key + '\n ' + localStorage[key]);
                    logs.push(`\n${getDate()},${getTime()},執行,點擊綠旗 ${key}工作區：${localStorage[key]}`);
                }
            });
    }
...
function clickSprite(targetElement) { // this function is for Bottom right corner, sprite area
    ...
    if (targetElement.className == "sprite-selector-item_sprite-image-inner_3oSwi" ||
        ...
        logs.push(`\n${getDate()},${getTime()},切換角色,切換角色`);
        logs.push(`\n${getDate()},${getTime()},畫布變更,`);
...
function getDbFile() {
    ...
    const logsFileRef = firebase.storage().ref(`${localStorage.identity}/${classno}/${userno}/Projects/${urlParams.get('p')}/${username}_${urlParams.get('p')}.csv`);
                ...
                .then(function (response) {
                    response.text().then(function (text) {
                        ...
                        if (urlParams.get("edit") == "true") {
                            ...
                            create('\uFEFF' + storedText + '\n\n' + getDate() + ',' + getTime() + ',編輯專案,' + urlParams.get('p') + logs.join(' '),
                            ...
                            create('\uFEFFDate,Time,Action,Details' + logs.join(' '),
                                username + '_' + urlParams.get('p') + '_' + new Date().toLocaleString() + '.csv',
                                'text/csv;charset=utf-8');
...
function setToken() {
    ...
    window.location.href = `${guiUrl}?token=${localStorage.token}&classno=${localStorage.classno}&no=${localStorage.no}&name=${localStorage.username}&p=${urlParams.get('p')}&i=${localStorage.identity}`;
}