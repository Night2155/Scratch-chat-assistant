...
class SBFileUploader extends React.Component {
    ...
    handleClick () {
/**
 * - 如果網址裡有 p= 這個參數，就走「從 Firebase Storage 載入學生專案」那條路。
 * - 如果網址裡有 ex= 這個參數，就走「載入 example 範例專案」那條路。
 */
let urlParams = new URLSearchParams(window.location.search);
        if (window.location.search.match('p') != null) {
            var sbfl = this;
            var request = new XMLHttpRequest();
            const config = { apiKey: "AIzaSyBYg4xnd5il5QcsYfJu1Zj89wdoYHqjlAo", authDomain: "scratch-ct-chatbot-2025.firebaseapp.com", projectId: "scratch-ct-chatbot-2025", storageBucket: "scratch-ct-chatbot-2025.firebasestorage.app", messagingSenderId: "890112360772", appId: "1:890112360772:web:7b9911703cbb6f8b171711", databaseURL: "https://scratch-ct-12a21.firebaseio.com"};
            if (!firebase.apps.length) {
                firebase.initializeApp(config);
            }
            // var sb3Name = urlParams.get('s');
            var sb3Path = urlParams.get("sb3");
            // var projName = urlParams.get('p');
            // var no = urlParams.get('no');
            // var classno = urlParams.get('classno');
            const fileRef = firebase.storage().ref(`${sb3Path}`);
            ...
            fileRef.getDownloadURL().then(function (url) {
                ...
                request.onload = function () {
                    ...
                    reader2.onload = function (e) {
                        ...
                        sbfl.props.vm.loadProject(e.target.result)
                            .then(() => {
                                console.log('VM load project success');

                                // ✅ 通知 script3：sb3 載入完成
                                if (window.onSb3Loaded) {
                                    window.onSb3Loaded();
                                } else if (window.hideLoading) {
                                    // 保險：沒有 onSb3Loaded 至少關掉遮罩
                                    window.hideLoading();
                                }
                            ...
                            .catch(err => {
                                console.error('VM load project failed:', err);
                                // 發生錯誤也把 loading 關掉，避免卡住
